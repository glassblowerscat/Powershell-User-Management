	...
	function Input-Alias ([string]$sam){
		Do {
			...
			If ($aliasResponse -ne "d"){
                ...
                Elseif ($emaildomain -eq '@crosswindsyouth.org'){
					$proxyAddress1 = $aliasResponse + '@crosswinds.org'
                    $proxyaddress2 = $aliasresponse + '@crosswindsyouth.org'
					Set-ADUser -Identity $sam -add @{ProxyAddresses=$proxyAddress1}
                    Set-ADUser -Identity $sam -add @{ProxyAddresses=$proxyAddress2}
					}
				Else {
		...
		Do {
			...
			If ($aliasResponse -ne "d"){
                ...
                Elseif ($emaildomain -eq '@crosswindsyouth.org'){
					$proxyAddress1 = $aliasResponse + '@crosswinds.org'
                    $proxyaddress2 = $aliasresponse + '@crosswindsyouth.org'
					Set-ADUser -Identity $sam -remove @{ProxyAddresses=$proxyAddress1}
                    Set-ADUser -Identity $sam -remove @{ProxyAddresses=$proxyAddress2}
					}
				Else {
					$proxyAddress = $aliasResponse + $emailDomain
